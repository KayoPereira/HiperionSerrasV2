...
# docker build -t hiperion_serras_v2 .
# docker run -d -p 80:80 -e RAILS_MASTER_KEY=<value from config/master.key> --name hiperion_serras_v2 hiperion_serras_v2
...
RUN apt-get update -qq && \
    apt-get install --no-install-recommends -y build-essential git libpq-dev libyaml-dev nodejs pkg-config && \
    rm -rf /var/lib/apt/lists /var/cache/apt/archives
